...
from utils.loss import CrossEntropyLoss2d, weighted_BCE_logits,unChangeSimilarity
from utils.utils import accuracy, SCDD_eval_all, AverageMeter

#Data and model choose
###############################################
from datasets import RS_ST as RS
from models.SICNet import SICNet as Net
NET_NAME = 'SICNet'
DATA_NAME = 'SECOND'
###############################################    
...
def train(train_loader, net, criterion, optimizer, scheduler, val_loader):
    ...
    criterion_uc = unChangeSimilarity().cuda()
    ...
    while True:
        ...
        for i, data in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            out_change1,out_change, outputs_A, outputs_B = net(imgs_A, imgs_B)
            assert outputs_A.size()[1] == RS.num_classes  
            loss_seg = criterion(outputs_A, label_A) * 0.5 +  criterion(outputs_B, label_B) * 0.5   
            loss_bn = weighted_BCE_logits(out_change, labels_bn)*0.5 + weighted_BCE_logits(out_change1, labels_bn)*0.5
            loss_uc = criterion_uc(outputs_A, outputs_B, labels_bn.squeeze()) 
