...
class Data(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.random_flip:
            img_A, img_B, label_A, label_B = transform.rand_rot90_flip_MCD4(img_A, img_B, label_A, label_B)
        img_A = self.normalize(img_A)
        img_B = self.normalize(img_B)

        return img_A, img_B, torch.from_numpy(label_A), torch.from_numpy(label_B)
    def __len__(self):
...
def extract_numbers(image_name):
    # 使用正则表达式提取图片名称中的数字部分
    numbers = [int(match) for match in re.findall(r'\d+', image_name)]
...
class Data_test_eval(data.Dataset):
    def __init__(self, test_dir):
        ...
        data_list = sorted(os.listdir(imgA_dir),key=extract_numbers)
        for it in data_list:
            if (it[-4:]=='.png'):
                img_A_path = os.path.join(imgA_dir, it)
                img_B_path = os.path.join(imgB_dir, it)
